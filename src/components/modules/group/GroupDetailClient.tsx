...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold ring-1 ${s.cls}`}
    >
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      >
        <span className={accent ? "text-emerald-100" : "text-[#006A4E]"}>
          {icon}
        </span>
      </div>
      <div>
...
function ReportButton({ groupId }: { groupId: number }) {
  ...
  async function handleDownload() {
    ...
    try {
      // FIX: generateDiplomaReport only accepts groupId — semester filtering
      // is handled internally by the service (shows previous semester).
      const result = await generateDiplomaReport(groupId);
...
export default function GroupDetailClient({ group }: { group: Group }) {
  ...
  const [activeTab, setActiveTab] = useState<"students" | "results">(
    "students",
  );
  ...
  const filteredResults =
    activeSem === "all"
      ? results
      : results.filter((r) => r.semesterName === activeSem);
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-50">
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                {totalStudents} student{totalStudents !== 1 ? "s" : ""} in this
                group
              </p>
            </div>
                    ...
                    >
                      ...
                      {pct === 0 ? (
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          ...
                          <span
                            className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${attBadgeCls(pct)}`}
                          >
                            {pct}%
                          </span>
                        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <span
                    key={s.label}
                    className={`text-xs font-bold px-3 py-1.5 rounded-full ring-1 ${s.cls}`}
                  >
                    {s.label}: {s.val}
                  </span>
                ))}
              ...
              {/* FIX: ReportButton now only takes groupId — no activeSem prop */}
              {results.length > 0 && <ReportButton groupId={group.id} />}
            </div>
                ...
                >
                  <thead>
                    <tr className="bg-slate-800 text-white text-xs">
                      <th className="text-left px-4 py-3.5 font-semibold tracking-wide w-[185px]">
                        Student
                      </th>
                      <th className="text-left px-3 py-3.5 font-semibold tracking-wide w-[82px]">
                        Roll
                      </th>
                      <th className="text-left px-3 py-3.5 font-semibold tracking-wide w-[68px]">
                        Sem.
                      </th>
                      <th className="text-left px-3 py-3.5 font-semibold tracking-wide w-[52px]">
                        Year
                      </th>
                      {SEM_LABELS.map((l) => (
                        <th
                          key={l}
                          className="text-center px-2 py-3.5 font-semibold w-[44px] text-slate-400"
                        >
                          {l}
                        </th>
                      ))}
                      <th className="text-center px-3 py-3.5 font-semibold tracking-wide w-[56px]">
                        Avg
                      </th>
                      <th className="text-center px-3 py-3.5 font-semibold tracking-wide w-[90px]">
                        Status
                      </th>
                    </tr>
                  </thead>
                          ...
                          >
                            ...
                            {GPA_KEYS.map((k) => (
                              <td key={k} className="px-2 py-3 text-center">
                                {r[k] !== null ? (
                                  <span
                                    className={`text-[11px] ${gpaColor(r[k])}`}
                                  >
